...
> **状态**：**待用户批准**；本文只制定迁移与验收方案，未获批准前不得停机、导入、刷新、调度、Aidso、切换或修改代码。
> **编制日期**：2026-07-16。
> **现行架构依据**：[`全域内容资产与观测架构方案_v3.3.md`](../全域内容资产与观测架构方案_v3.3.md)。
> **数据语义参考**：[`全域内容资产与观测架构方案_v3.2.md`](../全域内容资产与观测架构方案_v3.2.md)。
> **总验收依据**：[`全域内容工作台验收矩阵_v2.md`](../全域内容工作台验收矩阵_v2.md)，执行 G0、T041–T075、T281–T296 的微信适用用例，并完成 T297 中微信页面巡检证据；T298–T300 仅属于项目首版完成；不得用旧矩阵替代。
...
1. **先等你批准**：没批准前不关服务、不导数据、不刷新、不调度、不调用 Aidso、不切流。
2. **把旧系统封存**：停掉抓取和调度，做一份带 `freeze_id`、哈希和时间的**只读、不可变标准答案冻结包**；之后所有比较只读这份包，任何变化都作废重冻。
3. **把 8774 当参考端**：只启动冻结包的参考实例；工作台固定使用 `8799`，两者隔离，不能把参考端当生产入口。
4. **把数据搬进新 Hub**：14 张跨系统核心表保持独立；微信关键词专属的 `search_*` 运行表另建，不能混进核心表。
5. **43 个操作逐个对账**：旧系统是 **42 条路由声明、43 个 HTTP 操作**，接口表一条不少；逐字段、逐状态、逐排序、逐落盘比较，零未解释差异才能过。
6. **可逆命令只做双写**：命令写新 Hub，同时写隔离的旧副本用于比对；真实外部抓取、真实调度和真实 Aidso 每个动作只调用一次，且必须先获用户批准。
7. **按原页面验收**：固定保存 7 条页面路由快照，只能用 Codex 内置浏览器；CSS、资源和 `provenance` 隔离，不能用新壳层覆盖旧业务证据。
8. **最后才切换和恢复**：G0、T041–T075、T281–T296 的微信适用用例全部通过，并完成 T297 中微信页面巡检证据，四项 P0 门禁全部通过，才允许 `compare → hub`；T298–T300 只计入项目首版完成；真实刷新、调度、Aidso 仍逐项请求批准，失败可立即回退。

> **一句话：先封存标准答案，再在隔离环境把 43 个操作和 7 个页面逐一复刻；没有用户批准和零差异证据，就不切换、不抓取、不调度。**
...
- 冻结参考端 `8774`、新兼容 API（由工作台 `8799` 提供业务入口）隔离运行；禁止再引入未批准的第三个业务端口；
...
每个复制到工作台的页面、正文或静态资源都必须带 `provenance`：`source_root`、`source_files`、`source_hash`、`copied_at`、`allowed_changes`。原源永久只读，只能复制到 `workbench/`；允许的变更仅为 API base、宿主挂载、CSS namespace 和必要安全修复，不做视觉改版，不改原字段、DOM、操作顺序或业务语义。
...
| 角色 | 地址 | 迁移期间职责 |
|---|---|---|
| 冻结参考端 | `127.0.0.1:8774` | 冻结后的标准答案，仅供读取和比较 |
| 新兼容 API | 由 `127.0.0.1:8799` 工作台提供 | 从新 Hub/运行库还原旧 `/api/...` 契约；不另开参考端口 |
...
- 冻结包或 `8774` 参考实例发生任何写入，冻结立即作废并重做；若旧实时目录或 `8765` 在 freeze 后被误启动并产生 post-freeze tail，按 P0 立即停止、隔离，该 tail 永不进入基线；只有冻结包 manifest/hash 与恢复 API 仍完全一致时，原 `freeze_id` 才可继续使用，否则必须重冻。
...
- 不显示假成功 Toast；
- 冻结参考端 `8774` 不作为用户正常操作入口，只作为比较标准答案。
...
每次冻结只生成一套不可混淆的**标准答案冻结包**；冻结后以只读挂载/权限和 manifest hash 保护，不允许覆盖、追加或回写：
...
- v3.3 的 14 张跨系统核心表保持独立、只承载跨系统事实：`contents`、`creators`、`content_identifiers`、`content_discoveries`、`search_snapshots`、`search_hits`、`metric_definitions`、`metric_observations`、`comments`、`comment_events`、`geo_answers`、`geo_source_relations`、`signals`、`production_jobs`。
- `audit_log`、`source_refs`、`provenance` 等只能称工程/审计支持，不得混称为 14 张核心表。
- 微信关键词模块运行表明确隔离为 `search_keyword_groups`、`search_keyword_settings`、`search_refresh_jobs`、`search_refresh_items`、`search_scheduler_state`；可补充微信专属扩展表，但所有扩展必须以 `system_key`/`platform` 隔离，不能冒充核心事实表。
- 每条核心事实必须带可回溯 `provenance`/`source_ref`；运行表只保存状态、命令、锁、重试和投影，不改变核心事实所有权。
...
```mermaid
flowchart LR
    Q["冻结请求集 / 页面真实请求"] --> O["冻结参考 API<br/>127.0.0.1:8774"]
    Q --> N["新兼容 API<br/>工作台 127.0.0.1:8799"]
    O --> C["接口专用规范化器"]
    N --> C
...
**POST/PATCH/DELETE 写接口**不能对真实生产状态各执行一次，否则会产生双重副作用。可逆命令采用“**新 Hub + 隔离旧副本双写**”：用户批准后，命令只提交一次，由 command 记录生成一个 `command_id`，新 Hub 写入真实候选状态，同时把同一命令写入隔离的旧副本 A；旧副本只用于回放和对账，绝不连接真实外部源。随后比较：

```text
旧冻结库的隔离副本 A     新 Hub/运行库 B
        ← 同一 command_id / 显式幂等键 →
  → 设置相同初始状态
  → 执行相同请求
  → 比较 HTTP 回执
...
正式导入、命令和重放都必须由调用方提供显式 `idempotency_key`；缺失不得自动生成隐式键。
...
不得对同一真实关键词同时触发旧抓取和新抓取；真实刷新、正式调度、Aidso 查询均须用户逐项批准，批准前只允许 dry-run/录制响应/模拟 Provider。
...
| R08 | `GET /api/agent/evidence/{evidence_id}` | source_ref/manifest/证据索引 | 合法 ID 内容、hash、来源；非法 ID 400、缺失 404；必须测试 `../../`、绝对路径、NUL 和软链接逃逸，全部拒绝 | R3 |
...
| R11 | `GET /api/article-content?path=...` | Markdown 资产 Repository | 原 Markdown 正文、受控相对路径、编码；非法路径 400、缺失 404；必须测试 `../../`、绝对路径、NUL 和软链接逃逸，全部拒绝 | R1 |
...
| R15 | `GET /api/keyword-manage` | 搜索运行库 | 页面只展示 `status='active' AND archived_at IS NULL` 的 312 个活跃词，覆盖 18 分组及顺序、每组关键词 ID/名称/备注/默认选择、刷新策略/状态/时间、商业价值、生命周期、锁、today/coverage/accounts/articles、总计；底层 457 条历史注册词另行对账，不得混入页面 | R1 |
...
| R21 | `GET /api/articles?...` | contents + hits + metrics + creator scores | total/page/page_size、文章顺序、ID/标题/URL/账号、read/like、hit/on-rank、score、发布时间、path/cover；7 种排序与全部筛选分页组合；非法 `sort` 必须回落到旧契约的 `reads`，响应中记录真实回落而不得 500、猜测排序或静默改变顺序 | R1 |
...
- R15：完整比较 18 个分组和页面可见的 312 个活跃关键词；同时独立核对底层 registry 保留 457 条历史注册词（312 active + 145 archived）。**457 底层历史注册词不等于 R15 页面 312 活跃词。**
- R21：分页遍历全量文章，并覆盖：
  - `reads`
  - `onRankDays`
  - `hitCount`
...
| W04 | `POST /api/keywords/{id}/topic` | 修改 topic | 比较 null/空值、trim 规则、列表与详情同步、重启持久化；None/空字符串/纯空白按契约写入 DB `NULL`，不得写成 `""` | W1 |
| W05 | `POST /api/keywords/{id}/note` | 修改备注 | 比较原文保存、空备注、Unicode、刷新后持久化、审计 | W1 |
| W06 | `POST /api/keywords/{id}/bucket` | 修改 bucket | 比较 bucket 值、未分组语义、列表/详情/管理页同步；None/空字符串/纯空白按契约写入 DB `NULL`，未分组只用 `NULL` 表示 | W1 |
...
验收前后必须各保存一份完整快照（HTML、可见文本、关键截图、请求清单、CSS/静态资源 manifest、`provenance`），固定路由为：`/`、`/keyword-manage`（redirect 到 `/?view=keyword-manage`）、`/keyword-turnover`、`/article-hit-detail`、`/article-hit-detail-demo`（redirect 到 `/article-hit-detail?article_id=art_749d447ea394`）、`/account-score-analysis`、`/account-score-formula`。`/article-hit-detail-demo` 的快照验收比较 redirect 语义及其目标路由结果，不把它作为独立 HTML 页面验收。快照只用于证据，不得把参考端内容回写工作台。

- 参考端 `8774` 的 HTML、CSS、JS、图片和 Markdown 资产只读；工作台 `8799` 使用独立 CSS 命名空间、静态资源根和 `provenance` 标记。
- CSS 不得跨端口注入或覆盖，页面不得通过 React catch-all 吞掉旧路由；每个快照必须能定位到对应接口、数据批次和来源证据。
...
### 13.0 四项 P0 门禁（任一失败即停止）

1. **G-R1 共享内容所有权**：只要共享 canonical 行存在任何非微信 namespace，微信导入不得覆盖 `title`、`creator_id`、`author_name`、`published_at`、`md_path`、`payload_json` 等共享字段。必备证据：非微信 namespace fixture、导入前后逐字段 diff、拒绝/跳过审计记录、重启后复读结果。
2. **G-R2 正式导入显式幂等键**：正式全量导入在 router、service、worker、state_command 等所有入口都必须显式接收并透传 `idempotency_key`，不得自动生成。必备证据：各入口缺键均失败的测试、键透传链路日志、同键重放与不同 payload 冲突测试。
3. **G-R3 command 与对账一致**：出现 `reconcile mismatch` 或 `asset_integrity.verified=false` 时，`command_runs` 不得为 `succeeded`，checkpoint 不得推进，且 audit、batch、command 三方记录必须一致。必备证据：两类失败 fixture、数据库状态断言、checkpoint 不推进断言、三方关联 ID/状态对账报告。
4. **G-R4 微信正文专属路径**：微信 `article-content`/`article-hit-detail` 必须从 `wechat_article_paths.asset_path` 读取，共享 canonical 不得回退 `contents.md_path`。必备证据：双路径 fixture、命中专属路径的读取日志、共享 `contents.md_path` 断言未被调用、路径越界/软链接安全测试。
...
- **42 条路由声明对应 43 个 HTTP 操作全部有实现、契约、测试和证据，零未解释差异、零遗漏**；
- **全部 43 个 HTTP 操作**都必须零未解释差异；不得按接口类别分级降级；`approved_exception` 仅允许书面登记的、非用户可见的技术差异，并且必须复测关闭。
- 全部错误状态和边界条件通过；
- 核心 GET 全量遍历通过，不只抽样；
...
- **G0、T041–T075、T281–T296 的微信适用用例全部 PASS，并完成 T297 中微信页面巡检证据**；不把项目级 T297 标为整项 PASS；
- **7 条页面路由完整**；
...
- 原微信页面未被重新设计，原功能和 7 条页面路由完整；
- 冻结旧数据可恢复、可启动、可作为只读标准答案；
- 42 条路由声明对应 43 个 HTTP 操作全部存在；
- 全部 43 个操作完成逐字段、逐错误、逐排序、逐落盘比较，零未解释差异；
- 所有写入完成隔离副本、副作用、显式幂等、重启和审计比较；
- 搜索快照、排名、词特征、正文和动态观测均可追溯；
- `legacy/compare/hub` 可切换且已演练回退；
- **G0、T041–T075、T281–T296 的微信适用用例全部 PASS，并完成 T297 中微信页面巡检证据**；不把项目级 T297 标为整项 PASS；
- 单词和小批刷新经用户批准的真实或受控链路通过。

### 18.2 项目首版完成

项目首版不能与微信模块就绪混称；除微信模块就绪条件外，还必须满足：

- **T001–T300 全部 PASS**，不得缩减、跳过或用旧矩阵替代；其中 T298–T300 只属于项目首版完成，不作为微信模块就绪条件；
- 统一首页、九个主导航页面、数据治理/系统状态/备份恢复/审计等验收范围全部完成；
- 七系统真实链路、备份恢复、回退和证据包齐全；
- scheduler 恢复后连续 7 天无未解决 P0/P1；
- 旧系统和原始历史不删除，旧服务从“生产依赖”降为“只读归档与紧急参考”。

> **最终判断：微信模块就绪只是微信迁移达到验收门槛；项目首版完成还必须满足全项目 T001–T300。两者不得混称。**
