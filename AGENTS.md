...
**Wiki 例外（用户已明确授权）**：母文章 Wiki 是唯一允许直接回写原始业务目录的模块。Wiki 的目录树、搜索、正文编辑、Markdown 删除和图片批量删除统一直接操作 `/Users/works14/Documents/output_md`；所有写入仍必须经过 Wiki Hub 服务、路径/软链接校验、原子写入、单写锁、版本与审计。微信关键词、小红书关键词及其他原始系统继续遵守只读边界。
