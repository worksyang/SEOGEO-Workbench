 ...
 * 复用 openArtByUrl 打开笔记抽屉, selectBlogger 跳转博主透视
 */
...
(function () {
  ...
  async function fetchArticles() {
    ...
    try {
      ...
      renderError('加载笔记列表失败: ' + e.message);
    } finally {
      loading = false;
    }
  }
  ...
  function renderTable(articles) {
    ...
    if (countEl) countEl.textContent = state.total + ' 篇笔记';

    if (!articles || articles.length === 0) {
      tbody.innerHTML = '<tr><td colspan="10" class="al-empty">没有符合条件的笔记</td></tr>';
      return;
    }
    ...
    articles.forEach(function (a, i) {
      ...
      var dataAttrs = 'data-article-id="' + esc(a.article_id || '') + '" ' +
        ...
        'data-work-type="' + esc(a.work_type || '') + '" ' +
        'data-rank="' + rank + '" ' +
        'data-account="' + esc(a.account_name) + '" ' +
        'data-liked-count="' + (a.liked_count != null ? a.liked_count : '') + '"\n        data-collected-count="' + (a.collected_count != null ? a.collected_count : '') + '"\n        data-comment-count="' + (a.comment_count != null ? a.comment_count : '') + '"\n        data-shared-count="' + (a.shared_count != null ? a.shared_count : '') + '"\n        data-read-count="' + (a.read_count != null ? a.read_count : '') + '" ' +
      ...
      html += '<tr class="al-art-row" ' + dataAttrs + '>' +
        ...
        '<td class="al-col-likes"><span class="al-num al-num-hi">' + fmtNum(a.liked_count) + '</span></td>' +
        '<td class="al-col-colls"><span class="al-num al-num-hi">' + fmtNum(a.collected_count) + '</span></td>' +
        '<td class="al-col-comments"><span class="al-num">' + fmtNum(a.comment_count) + '</span></td>' +
        '<td class="al-col-shared"><span class="al-num">' + fmtNum(a.shared_count) + '</span></td>' +
  ...
  function renderAcctDropdown() {
    ...
    if (filtered.length === 0) {
      list.innerHTML = '<div class="al-acct-empty">没有匹配的博主</div>';
      return;
    }
    ...
    html += '<div class="al-acct-option' + (state.account === '' ? ' selected' : '') + '" data-acct="" data-name="全部博主">' +
      '<span class="al-acct-option-name">全部博主</span>' +
    ...
    if (filtered.length > 200) {
      html += '<div class="al-acct-empty">还有 ' + (filtered.length - 200) + ' 个博主，请输入关键词搜索…</div>';
    }
    ...
    list.querySelectorAll('.al-acct-option').forEach(function (opt) {
      opt.addEventListener('click', function () {
        ...
        state.accountName = opt.dataset.name || '全部博主';
        state.page = 1;
        closeAcctDropdown();
        updateAcctTrigger();
        fetchArticles();
      ...
      tbody.addEventListener('click', function (e) {
        ...
        if (row) {
          ...
          var meta = {
            ...
            work_type: row.dataset.workType || '',
