(function () {
  ...
  function articleHitDetailHref(article) {
    if (article?.article_id) return `/legacy/xhs/article-hit-detail?article_id=${encodeURIComponent(article.article_id)}`;
    if (hasRealUrl(article?.url)) return `/legacy/xhs/article-hit-detail?url=${encodeURIComponent(article.url)}`;
  ...
  function renderAll(payload) {
    ...
    back.href = `/legacy/xhs/monitor.html?keyword=${keywordParam}`;
