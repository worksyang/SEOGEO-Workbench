(function () {
  ...
  function articleHitDetailHref(article) {
    ...
    if (hasRealUrl(article?.url)) return wechatAuxUrl('/article-hit-detail', { url: article.url });
  ...
  async function load() {
    ...
    try {
      ...
      let keyword = null;
      if (keywordId) {
        keyword = await fetchJson(`${KEYWORD_DETAIL_API_BASE}/${encodeURIComponent(keywordId)}`);
      } else {
        payload = await fetchJson(BOOTSTRAP_URL);
        const keywords = payload.keywords || [];
        const summary = keywords.find(item => item.keyword === keywordText) || keywords[0];
        if (summary?.keyword_id) {
          keyword = await fetchJson(`${KEYWORD_DETAIL_API_BASE}/${encodeURIComponent(summary.keyword_id)}`);
        }
      }
  ...
  function renderHero() {
    ...
    const explain = mature
      ? `${pct(turnover.rate)} 的意思：过去${turnover.windowDays}天里，平均每次抓取后，${averageShare}上榜文章和上一次不一样。它不是阅读量，也不是热度，只看“榜单里的文章有没有换人”。`
